...
func (s *SQLiteStore) CreatePeer(peer *models.Peer) error {
	...
	_, err := s.db.Exec(`
		INSERT INTO peers (id, interface_id, type, name, public_key, private_key_encrypted, preshared_key_encrypted, address, allowed_ips, client_allowed_ips, dns, endpoint, persistent_keepalive, alert_on_connect, alert_on_disconnect, alert_emails, enabled, expires_at, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		peer.AlertOnConnect, peer.AlertOnDisconnect, peer.AlertEmails,
...
func (s *SQLiteStore) GetPeer(id string) (*models.Peer, error) {
	...
	err := s.db.QueryRow(`
		SELECT id, interface_id, type, name, public_key, private_key_encrypted, preshared_key_encrypted, address, allowed_ips, client_allowed_ips, dns, endpoint, persistent_keepalive, alert_on_connect, alert_on_disconnect, alert_emails, enabled, expires_at, last_handshake, transfer_rx, transfer_tx, created_at, updated_at
		FROM peers WHERE id = ?`, id,
	).Scan(
		&peer.ID, &peer.InterfaceID, &peer.Type, &peer.Name, &peer.PublicKey,
		&peer.PrivateKeyEncrypted, &psk, &peer.Address, &peer.AllowedIPs, &peer.ClientAllowedIPs, &peer.DNS, &endpoint,
		&peer.PersistentKeepalive, &peer.AlertOnConnect, &peer.AlertOnDisconnect, &peer.AlertEmails, &peer.Enabled,
...
func (s *SQLiteStore) UpdatePeer(peer *models.Peer) error {
	...
	_, err := s.db.Exec(`
		UPDATE peers SET interface_id=?, type=?, name=?, public_key=?, private_key_encrypted=?, preshared_key_encrypted=?, address=?, allowed_ips=?, client_allowed_ips=?, dns=?, endpoint=?, persistent_keepalive=?, alert_on_connect=?, alert_on_disconnect=?, alert_emails=?, enabled=?, expires_at=?, last_handshake=?, transfer_rx=?, transfer_tx=?, updated_at=?
		WHERE id=?`,
